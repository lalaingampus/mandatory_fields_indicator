...
def create_banner(output_path):
    ...
    # Glow 1: Top-Left Indigo (#4F46E5)
    glow1 = Image.new('RGBA', (W, H), (0, 0, 0, 0))
    g1_draw = ImageDraw.Draw(glow1)
    g1_draw.ellipse([-200 * SCALE, -200 * SCALE, 750 * SCALE, 600 * SCALE], fill=(79, 70, 229, 190))
    glow1 = glow1.filter(ImageFilter.GaussianBlur(radius=160 * SCALE))
    
    # Glow 2: Bottom-Right Coral (#E11D48)
    glow2 = Image.new('RGBA', (W, H), (0, 0, 0, 0))
    g2_draw = ImageDraw.Draw(glow2)
    g2_draw.ellipse([600 * SCALE, 150 * SCALE, 1400 * SCALE, 800 * SCALE], fill=(225, 29, 72, 170))
    glow2 = glow2.filter(ImageFilter.GaussianBlur(radius=180 * SCALE))
    
    # Glow 3: Top-Right Sapphire Accent (#0EA5E9)
    glow3 = Image.new('RGBA', (W, H), (0, 0, 0, 0))
    g3_draw = ImageDraw.Draw(glow3)
    g3_draw.ellipse([800 * SCALE, -250 * SCALE, 1450 * SCALE, 350 * SCALE], fill=(14, 165, 233, 140))
    glow3 = glow3.filter(ImageFilter.GaussianBlur(radius=150 * SCALE))

    # Glow 4: Bottom-Left Amber Accent (#F59E0B)
    glow4 = Image.new('RGBA', (W, H), (0, 0, 0, 0))
    g4_draw = ImageDraw.Draw(glow4)
    g4_draw.ellipse([-100 * SCALE, 380 * SCALE, 450 * SCALE, 750 * SCALE], fill=(245, 158, 11, 120))
    ...
    f_title = ImageFont.truetype(font_bold, 42 * SCALE)
    ...
    card_draw.rounded_rectangle([card_x1, card_y1, card_x2, card_y2], radius=24*SCALE, fill=(15, 23, 42, 155), outline=(255, 255, 255, 55), width=2*SCALE)
    ...
    if not os.path.exists(icon_path):
        icon_path = r"C:\Users\bottl\Documents\Custom Modul Odoo\mandatory_fields_indicator\static\description\icon.png"
    ...
    # Version Pill Badges
    v_badges = [
        ("ODOO 16.0 • 17.0 • 18.0", (79, 70, 229), (255, 255, 255), (199, 210, 254)),
        ("COMMUNITY & ENTERPRISE", (16, 185, 129), (255, 255, 255), (110, 231, 183)),
        ("8 VISUAL STYLES", (225, 29, 72), (255, 255, 255), (254, 205, 211)),
    ]
    ...
    # Title
    title_y = by + 44 * SCALE
    draw.text((content_x, title_y), "Mandatory Fields Visual Indicator", fill=(255, 255, 255), font=f_title)
    
    # Subtitle
    sub_y = title_y + 60 * SCALE
    draw.text((content_x, sub_y), "Clear, Modern & Configurable Highlights for Required Form Fields", fill=(203, 213, 225), font=f_subtitle)
    ...
    features = [
        # (Title, Accent Color)
        ("Modern Left-Accent Bar", (79, 70, 229)),
        ("Soft Pastel Tint", (245, 158, 11)),
        ("Explicit [Required] Badge", (225, 29, 72)),
        ("Cyber Indigo Glow", (14, 165, 233)),
        ("Coral Pulse Dot & Tag", (244, 63, 94)),
        ("Executive Amber Gold", (217, 119, 6)),
        ("Sapphire Glassmorphism", (59, 130, 246)),
        ("Classic Asterisk Only", (239, 68, 68)),
    ]
    ...
    for idx, (label, accent_col) in enumerate(features):
        ...
        draw.rounded_rectangle([cx1, cy1, cx2, cy2], radius=10*SCALE, fill=(30, 41, 59, 220), outline=(*accent_col[:3], 120), width=int(1.5*SCALE))
...
if __name__ == '__main__':
    ...
    target = os.path.join(script_dir, "mandatory_fields_indicator", "static", "description", "banner.png")
